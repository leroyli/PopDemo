...
static float const kPopoverViewWidth       = 200;
static float const kPopoverViewHeight      = 200;
...
/*! @brief hide */
- (void)hide {
    [[NSNotificationCenter defaultCenter] postNotificationName:MMPopviewDissmissNotification object:nil];
...
- (void)initialize {
    ...
    _windowWidth  = CGRectGetWidth(self.keyWindow.bounds);
    _windowHeight = CGRectGetHeight(self.keyWindow.bounds);
...
- (void)showToPoint:(CGPoint)toPoint {
    ...
    CGFloat currentW = kPopoverViewWidth; // 宽度通过计算获取最大值
    CGFloat currentH = kPopoverViewHeight + kPopoverViewArrowHeight;
...
- (void)showWithAnimation{
    ...
    self.layer.anchorPoint = CGPointMake(self.pointArrow.x/kPopoverViewWidth, _isUpward ? 0.f : 1.f);
    self.frame = oldFrame;
    self.transform = CGAffineTransformMakeScale(0.01f, 0.01f);
    [UIView animateWithDuration:0.25f animations:^{
...
- (void)pickerView:(UIPickerView *)pickerView didSelectRow:(NSInteger)row inComponent:(NSInteger)component{
    NSLog(@">>>>>>>>>>>>>MMPoperView:you have selected %lu",row);
    if (self.delegate && [self.delegate respondsToSelector:@selector(MMPopverView:selectedIndex:)]) {
        [self.delegate MMPopverView:self selectedIndex:row];
    }
}
